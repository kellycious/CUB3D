...
/*   Updated: 2023/05/09 23:16:38 by fwong            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_check_elements(t_map *map, t_elements *elements)
{
	...
	while (i < map->height)
	{
		...
		while (map->cub[i][j] == ' ' || map->cub[i][j] == '\t')
			j++;
		ft_assign_elements(map,  elements, i, j);
		i++;
	}
}
...
void	ft_assign_elements(t_map *map, t_elements *element, int i, int j)
{
	if (ft_strncmp(map->cub[i] + j, "NO ", 3) && element->no == false)
		...
		element->floor	= true;
			...
			|| (ft_strncmp(map->cub[i] + j, "C", 1) && element->ceiling == true)
			|| (cft_strncmp(map->cub[i] + j, "F", 1) && element->floor == true))
				ft_elements_error(map->cub[i][j], map->cub[i][j + 1], map->cub[i][j + 2]);
}

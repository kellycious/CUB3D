...
/*   Updated: 2023/05/09 23:14:20 by fwong            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_assign_elements(t_map *map, t_elements *element, int i, int j)
{
	if (ft_strncmp(map->cub[i] + j, "NO ", 3) && element->no == false)
		element->no = true;
	else if (ft_strncmp(map->cub[i] + j, "NO ", 3)) && element->so == false)
		element->so = true;
	else if (ft_strncmp(map->cub[i] + j, "NO ", 3)) && element->we == false)
		element->we = true;
	else if (ft_strncmp(map->cub[i] + j, "NO ", 3) && element->ea == false)
		element->ea = true;
	else if (ft_strncmp(map->cub[i] + j, "NO ", 3) && element->ceiling == false)
		element->ceiling = true;
	else if (ft_strncmp(map->cub[i] + j, "NO ", 3)&& element->floor == false)
		element->floor	= true;

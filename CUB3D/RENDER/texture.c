...
void	draw_texture(t_map *game, int x, int y)
{
	y = game->ray->starty - 1;
	init_texture(game);
	game->tex->step = 1.0 * game->txt[NORTH]->height / game->ray->line_height;
	game->tex->texx = (int)game->tex->wallx * (double)(game->txt[game->tex->dir]->width);
	if ((game->ray->hit_dir == NORTH && game->ray->dir.x > 0)
		|| (game->ray->hit_dir == SOUTH && game->ray->dir.y < 0))
		game->tex->texx = game->txt[game->tex->dir]->width - game->tex->texx - 1;

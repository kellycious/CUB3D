...
/*   Updated: 2023/05/28 15:19:12 by fwong            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../LIB/cub3d.h"

int	ft_(t_map *map, bool mappy)
{
	int		i;
	int		j;
	...
	return (ft_map_not_well_formated, 0);
}
...
int	ft_flood_fill(t_map *map, bool player)
{
	...
	while (map->map_fill[i])
	{
		...
		{
			if (i > 0 && ft_check_player(map, i, j) == 1)
			{
				player = true;
